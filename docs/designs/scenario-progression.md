...
Guidelines for creating scenarios in any language. Each language should have scenarios at every level, exercising that language's specific debugging patterns and common footguns.
...
1. **`scenario.json`** — name, language, timeout, budget, test commands
2. **`prompt.md`** — natural language bug description, no tool hints, no debugging instructions
3. **`src/`** — buggy source code + visible failing test
4. **`hidden/`** — oracle validation test the agent never sees

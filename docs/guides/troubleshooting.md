# Troubleshooting Agent Lens

Start with `agent-lens doctor` to check your setup before diving into specific issues.
...
**Symptom**: `LaunchError: Failed to download js-debug adapter` or missing adapter at `~/.agent-lens/adapters/js-debug/`

**Fix**:
```bash
# Clear the cache and retry
rm -rf ~/.agent-lens/adapters/js-debug/
agent-lens launch "node app.js"  # triggers re-download
```

If the download fails due to network restrictions, download manually from the [js-debug releases](https://github.com/microsoft/vscode-js-debug/releases) and extract to `~/.agent-lens/adapters/js-debug/`.
...
**Fix**:
```bash
rm -rf ~/.agent-lens/adapters/codelldb/
agent-lens launch "cargo run"  # triggers re-download
```
...
**Fix**:
- Agent Lens allocates random ports in the 4000–5000 range. If all are in use, wait and retry.
...
**Fix**:
- Increase timeout with `--timeout` flag (in ms): `agent-lens continue --timeout 30000`
- Check if the debugger process started: `agent-lens status`
...
- Run `agent-lens doctor` to verify the debugger version
...
**Fix**:
- Use absolute paths for breakpoints: `agent-lens break /full/path/to/file.py:25`
...
agent-lens eval "x" --session abc123
```
...
```bash
agent-lens launch "pytest tests/" --framework pytest
```
...
```bash
agent-lens launch "python app.py" --framework none
```
...
Agent Lens compresses older history automatically as sessions grow. The most recent stop always gets full detail. Older stops show summary information.
...
No adapter is registered for this file extension. Check `agent-lens doctor` for supported languages. Use `--language` to force a specific adapter.

### "Session is in 'running' state, expected 'stopped'"

Most debugging commands (eval, vars, step) require the program to be paused. Use `agent-lens continue` first to run to a breakpoint.

### "Failed to connect to debugger on port XXXX"

The debugger process started but isn't listening yet. This usually means:
1. The process crashed immediately — check `agent-lens status` for stderr output
...
1. Run `agent-lens doctor` and include the output
2. Check daemon logs: `cat ~/.agent-lens/daemon.log`
3. File an issue: https://github.com/anthropics/agent-lens/issues

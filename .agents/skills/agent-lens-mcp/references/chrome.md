...
## Quick Start
```
chrome_start(url: 'http://localhost:3000', profile: 'agent-lens')
...
```
chrome_start(profile: 'agent-lens', url: 'http://localhost:3000')
```
Creates a separate Chrome with its own user-data-dir under `~/.agent-lens/chrome-profiles/agent-lens`. Independent cookies, storage, login state.
...
1. `chrome_start(profile: 'agent-lens')` — isolated instance, no conflict (recommended)

...
| `cli` | `agent-lens` CLI on PATH + skill installed in workspace |
...
**The skill is installed like a real project.** The agent-lens skill is placed at `.claude/skills/agent-lens/SKILL.md` in the workspace — the same mechanism a real user would use — not injected via system prompt flags.
...
The harness copies `src/` into a fresh temp directory, installs the skill at `.claude/skills/agent-lens/`, git-inits the workspace, runs setup commands, then hands control to the agent. After the agent exits, `hidden/` is copied in and the validation command runs.
